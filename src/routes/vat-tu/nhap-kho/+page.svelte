<script>
	...
	/** @type {{ data: { vat_tu_list: any[], lich_su_nhap: any[] }, form: any }} */
	let { data, form } = $props();
	...
	let filteredVatTu = $derived(
		data.vat_tu_list.filter((vt) => {
			...
			return vt.ten_vat_tu.toLowerCase().includes(searchQuery.toLowerCase());
		})
	);
	...
	function getVatTuName(/** @type {string} */ id) {
		const vt = data.vat_tu_list.find((v) => v.id.toString() === id);
	...
	$effect(() => {
		if (form?.success) {
			showToast(form.message || 'Nhập kho thành công!', 'success');
	...
	let dropdownFiltered = $derived(
		data.vat_tu_list.filter((vt) => {
			...
			return (
				vt.ten_vat_tu.toLowerCase().includes(vtSearch.toLowerCase()) ||
				(vt.yeu_cau_ky_thuat || '').toLowerCase().includes(vtSearch.toLowerCase())
			);
		})
	);
...
{#if toast.show}
	...
	>
		...
		>
			<span
				class="material-symbols-outlined text-{toast.type === 'success' ? 'emerald' : 'rose'}-600"
			>
...
<div class="space-y-6">
	...
	<div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
		<div>
			<h1 class="text-2xl font-bold text-slate-900">Nhập kho vật tư</h1>
			<p class="mt-0.5 text-sm text-slate-500">
				Ghi nhận hàng hóa nhập vào kho, cập nhật tồn kho tự động.
			</p>
		</div>
	</div>
	...
	<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
		...
		<div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
			...
			<p class="mt-1 text-xl font-bold text-emerald-600">{data.lich_su_nhap.length}</p>
		</div>
		<div class="rounded-xl border border-blue-100 bg-blue-50 p-4 shadow-sm">
			<p class="text-xs font-semibold tracking-wider text-blue-600 uppercase">Loại đang nhập</p>
			<p class="mt-1 text-xl font-bold text-blue-700">{soLoaiVatTu}</p>
		</div>
		<div class="rounded-xl border border-blue-100 bg-blue-50 p-4 shadow-sm">
			<p class="text-xs font-semibold tracking-wider text-blue-600 uppercase">Tổng SL nhập</p>
			<p class="mt-1 text-xl font-bold text-blue-700">{tongSoLuong}</p>
		</div>
	</div>
		...
		<div class="lg:col-span-3">
			...
			>
				...
				<div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
					<h2 class="text-sm font-bold text-slate-900 uppercase">📦 Phiếu Nhập Kho</h2>
					<button
						type="button"
						onclick={addItem}
						class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 hover:bg-emerald-100"
					>
					...
					<table class="w-full text-sm whitespace-nowrap">
						...
						>
							<tr>
								...
								<th class="w-28 border-b border-slate-100 px-4 py-3">Tồn hiện tại</th>
								<th class="w-24 border-b border-slate-100 px-4 py-3">Số lượng nhập *</th>
								<th class="w-10 border-b border-slate-100 px-4 py-3"></th>
							</tr>
						</thead>
						<tbody class="divide-y divide-slate-100">
							{#each items as item (item.id)}
								{@const selectedVt = data.vat_tu_list.find(
									(v) => v.id.toString() === item.vat_tu_id
								)}
								<tr transition:slide={{ duration: 150 }} class="hover:bg-slate-50/50">
									<td class="px-4 py-2">
										<div class="relative min-w-[220px]">
											...
											<div
												class="flex cursor-pointer items-center justify-between rounded border border-slate-200 bg-white px-2 py-1.5 text-xs hover:border-slate-300"
												onclick={() => {
												...
												>
													...
													<ul class="max-h-60 overflow-y-auto p-1">
														{#each dropdownFiltered as vt}
															...
															<li
																class="cursor-pointer rounded p-2 text-xs hover:bg-[#1e5ed4]/5 {item.vat_tu_id ===
																vt.id.toString()
															...
															>
																<div class="flex items-center justify-between">
																	...
																	<span
																		class="ml-2 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500"
																	>
									...
									<td class="px-4 py-2 text-center font-mono text-xs font-bold text-slate-600">
									...
									<td class="px-4 py-2">
										<input
											...
											required
											class="w-full rounded border border-slate-200 bg-white px-2 py-1.5 text-center font-mono text-xs font-bold focus:border-emerald-500 focus:outline-none"
										/>
									</td>
				...
				<div class="border-t border-slate-100 p-5">
					...
					<textarea
						...
						name="ghi_chu"
						rows="2"
						placeholder="Ghi chú về lô hàng, nhà cung cấp, đợt mua..."
						class="w-full resize-none rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
					></textarea>
					<div class="mt-4 flex justify-end">
						<button
							type="submit"
							disabled={isSubmitting || items.every((i) => !i.vat_tu_id)}
							class="flex items-center gap-2 rounded-lg bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
						>
		...
		<div class="lg:col-span-2">
			<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
				<div class="border-b border-slate-100 px-5 py-4">
					<h2 class="text-sm font-bold text-slate-900 uppercase">📋 Lịch sử nhập gần đây</h2>
				</div>
				<div class="divide-y divide-slate-100">
					{#if data.lich_su_nhap.length === 0}
						<div class="py-10 text-center text-sm text-slate-400">Chưa có lịch sử nhập kho.</div>
					{:else}
						{#each data.lich_su_nhap.slice(0, 20) as entry}
