<script>
	...
	/** @typedef {{ id: number, ten_nganh: string, ma_nganh: string, khoa_id: number, khoa: Khoa }} Nganh */

	/** @type {{ data: { nganh: Nganh[], khoa_list: Khoa[] }, form: { success?: boolean, error?: string, message?: string } }} */
	let { data, form } = $props();
	...
	let filteredNganh = $derived(
		data.nganh.filter(
			(item) =>
				item.ten_nganh.toLowerCase().includes(searchQuery.toLowerCase()) ||
				(item.ma_nganh && item.ma_nganh.toLowerCase().includes(searchQuery.toLowerCase())) ||
				(item.khoa && item.khoa.ten_khoa.toLowerCase().includes(searchQuery.toLowerCase()))
		)
	);
	...
	function openAddModal() {
		...
		maNganh = '';
		khoaId = '';
		if (data.khoa_list && data.khoa_list.length > 0) {
			khoaId = data.khoa_list[0].id.toString();
		}
		showModal = true;
	}

	function openEditModal(/** @type {Nganh} */ item) {
		editingItem = item;
		tenNganh = item.ten_nganh || '';
		maNganh = item.ma_nganh || '';
		khoaId = item.khoa_id ? item.khoa_id.toString() : '';
		showModal = true;
	}
	...
	function exportExcel() {
		const dataToExport = filteredNganh.map((n, index) => {
			return {
				STT: index + 1,
				'Mã ngành': n.ma_nganh || '',
				'Tên ngành': n.ten_nganh,
				'Thuộc Khoa': n.khoa?.ten_khoa || ''
			};
		});

		const worksheet = xlsx.utils.json_to_sheet(dataToExport);
		const workbook = xlsx.utils.book_new();
		xlsx.utils.book_append_sheet(workbook, worksheet, 'Nganh');
		xlsx.writeFile(workbook, 'Danh_Sach_Nganh.xlsx');
	}
</script>
...
<div class="space-y-6">
	...
	<div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
		<div>
			<h1 class="text-2xl font-bold text-slate-900">Quản lý Ngành</h1>
			<p class="mt-0.5 text-sm text-[#64748b]">
				Cấu trúc và quản lý các ngành học, khoa phụ trách.
			</p>
		</div>
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-3">
		...
		>
			...
			<div>
				<p class="text-xs font-semibold tracking-wider text-slate-500 uppercase">TỔNG SỐ NGÀNH</p>
				<p class="text-xl font-bold text-slate-900">{filteredNganh.length}</p>
			</div>
		</div>
		<div
			class="flex items-center space-x-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"
		>
			...
			<div>
				...
				<p class="text-xl font-bold text-emerald-600">{data.khoa_list.length}</p>
			</div>
		</div>
	</div>
	...
	<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
			...
			<div class="flex flex-wrap items-center justify-between gap-4">
				...
				<div class="relative w-full md:w-80">
					...
					<input
						...
						placeholder="Tìm kiếm ngành theo tên, mã, tên khoa..."
			...
			<table class="w-full border-collapse text-left">
				<thead>
					<tr class="bg-slate-50 text-[11px] font-bold tracking-wider text-slate-500 uppercase">
						<th class="border-b border-slate-100 px-6 py-3">MÃ NGÀNH</th>
						<th class="border-b border-slate-100 px-6 py-3">TÊN NGÀNH</th>
						<th class="border-b border-slate-100 px-6 py-3">KHOA QUẢN LÝ</th>
						<th class="border-b border-slate-100 px-6 py-3 text-center">THAO TÁC</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-slate-100">
					{#each filteredNganh as item (item.id)}
						<tr class="group transition-colors hover:bg-slate-50">
							...
							<td class="px-6 py-4 whitespace-nowrap">
								{#if item.khoa}
									<div class="flex items-center space-x-1.5 text-sm font-medium text-slate-600">
										<span class="material-symbols-outlined text-base text-[#1e5ed4]">domain</span>
										<span>{item.khoa.ten_khoa}</span>
									</div>
								{:else}
									<span class="text-sm text-slate-400 italic">Chưa phân khoa</span>
								{/if}
							</td>
							<td class="px-6 py-4 whitespace-nowrap">
								<div
						...
						<tr>
							<td colspan="4" class="px-6 py-20 text-center">
								<div class="flex flex-col items-center justify-center space-y-3 text-slate-400">
									<span class="material-symbols-outlined text-5xl opacity-40">layers</span>
									<p class="text-sm font-medium text-slate-500">Không tìm thấy ngành nào.</p>
								</div>
							</td>
						</tr>
					{/each}
		...
		<div class="flex items-center justify-between border-t border-slate-100 bg-white px-6 py-4">
			<p class="text-sm text-slate-500">
				...
				<span class="font-medium text-slate-800">{filteredNganh.length}</span> của
				<span class="font-medium text-slate-800">{data.nganh.length}</span> mục
			</p>
...
{#if showModal}
	...
	>
		...
		<div
			...
			class="relative w-full max-w-xl transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all"
			onclick={(e) => e.stopPropagation()}
		>
			<div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
				<h2 class="text-lg font-bold text-slate-900">
					{editingItem ? 'Cập nhật Ngành' : 'Thêm Ngành Mới'}
				</h2>
				<button
					aria-label="Đóng"
					onclick={closeModal}
				...
				<div class="grid grid-cols-2 gap-5">
					<div class="col-span-2">
						<label
							...
							for="ten_nganh">Tên ngành <span class="text-rose-500">*</span></label
						>
						<input
							type="text"
							id="ten_nganh"
					...
					<div class="col-span-2 sm:col-span-1">
						<label class="mb-1.5 block text-xs font-semibold text-slate-700 uppercase" for="khoa_id"
							>Khoa quản lý</label
						>
						<select
							id="khoa_id"
						...
						>
							<option value="" disabled selected={!khoaId}>-- Chọn Khoa --</option>
