<script>
	...
	/** @type {{ data: { he_dao_tao: HeDaoTao[] }, form: { success?: boolean, error?: string, message?: string } }} */
	let { data, form } = $props();
	...
	let filteredHeDaoTao = $derived(
		data.he_dao_tao.filter((item) => item.ten_he.toLowerCase().includes(searchQuery.toLowerCase()))
	);
	...
	function openEditModal(/** @type {HeDaoTao} */ item) {
		editingItem = item;
		tenHe = item.ten_he || '';
		showModal = true;
	}
	...
	function exportExcel() {
		const dataToExport = filteredHeDaoTao.map((h, index) => ({
			STT: index + 1,
			'Tên hệ đào tạo': h.ten_he
		}));

		const worksheet = xlsx.utils.json_to_sheet(dataToExport);
		const workbook = xlsx.utils.book_new();
		xlsx.utils.book_append_sheet(workbook, worksheet, 'HeDaoTao');
		xlsx.writeFile(workbook, 'Danh_Sach_He_Dao_Tao.xlsx');
	}
</script>
...
<div class="space-y-6">
	...
	<div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
		<div>
			<h1 class="text-2xl font-bold text-slate-900">Quản lý Hệ đào tạo</h1>
			<p class="mt-0.5 text-sm text-[#64748b]">Hiển thị và thêm mới các hệ đào tạo trong trường.</p>
		</div>
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
		...
		>
			...
			<div>
				<p class="text-xs font-semibold tracking-wider text-slate-500 uppercase">TỔNG HỆ ĐÀO TẠO</p>
				<p class="text-xl font-bold text-slate-900">{filteredHeDaoTao.length}</p>
			</div>
		</div>
	</div>
	...
	<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
			...
			<div class="flex flex-wrap items-center justify-between gap-4">
				...
				<div class="relative w-full md:w-80">
					...
					<input
						...
						placeholder="Tìm kiếm hệ đào tạo theo tên..."
			...
			<table class="w-full border-collapse text-left">
				<thead>
					<tr class="bg-slate-50 text-[11px] font-bold tracking-wider text-slate-500 uppercase">
						<th class="border-b border-slate-100 px-6 py-3">TÊN HỆ ĐÀO TẠO</th>
						<th class="border-b border-slate-100 px-6 py-3 text-right">THAO TÁC</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-slate-100">
					{#each filteredHeDaoTao as item (item.id)}
						...
						<tr>
							<td colspan="2" class="px-6 py-20 text-center">
								<div class="flex flex-col items-center justify-center space-y-3 text-slate-400">
									...
									<p class="text-sm font-medium text-slate-500">Không tìm thấy hệ đào tạo nào.</p>
								</div>
							</td>
						</tr>
					{/each}
		...
		<div class="flex items-center justify-between border-t border-slate-100 bg-white px-6 py-4">
			<p class="text-sm text-slate-500">
				...
				<span class="font-medium text-slate-800">{filteredHeDaoTao.length}</span> của
				<span class="font-medium text-slate-800">{data.he_dao_tao.length}</span> mục
			</p>
		...
		>
			<div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
				<h2 class="text-lg font-bold text-slate-900">
					{editingItem ? 'Cập nhật Hệ đào tạo' : 'Thêm Hệ đào tạo Mới'}
				</h2>
				<button
					aria-label="Đóng"
					onclick={closeModal}
			...
			>
				...
				<div class="space-y-4">
					<div>
						<label class="mb-1.5 block text-xs font-semibold text-slate-700 uppercase" for="ten_he"
							>Tên hệ đào tạo <span class="text-rose-500">*</span></label
						>
						<input
							type="text"
							id="ten_he"
